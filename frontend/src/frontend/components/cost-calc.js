...
export default {
	...
	methods: {
		...
		buttonClick() {
			...
			this.scrollToTop();
			this.sendAjaxRequestToWordPress();
		},
		sendAjaxRequestToWordPress() {
			const getTotalInfoDiv = this.$refs.getTotalInfo;
			let innerHTMLContent = getTotalInfoDiv.innerHTML;
			 innerHTMLContent = innerHTMLContent.replace(/class="[^"]*"/g, '').replace(/<!---->/g, '');

			console.log(innerHTMLContent);
			axios.post('your-ajax-endpoint', {htmlContent: innerHTMLContent})
				.then(response => {
